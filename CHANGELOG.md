...
- `report health-correlations [--since]` (#17): symptom × meal-item and symptom × meal-tag co-occurrence counts (`via: "item"|"tag"`) within the `suggest.windows` config — one key drives the suggester and the report. Windows are directional for the report: next-day counts a symptom on the calendar day after the meal, never before (the suggester stays symmetric). Output carries `windows` and an explicit co-occurrence-not-causation `note`; `--since` (validated ISO) applies to both sides of a pair. JSON or `--human` markdown. The slice also lands the remaining review cleanups: the health-kind set is now a kinds-registry flag (`KindDef.health` → exported `HEALTH_KINDS`) consumed by the suggester, med-adjacency, and this report; the tasks report derives its non-terminal statuses from the registry (`nonTerminalStatuses`); and wikilink/`--project` reference resolution share one rule (`resolveNodeRef`).
