...
- For time-anchored kinds, `occurred_at` is the canonical query timestamp: the CLI maintains `occurred_at = starts_at` for events and `occurred_at = started_at` for medications as an **invariant** — it applies on `add` and re-fires on any update that changes the source field (a rescheduled event moves with it; a corrected regimen start moves with it). Passing `--occurred-at` explicitly on these kinds (add or update) is `INVALID_ARGS`; the payload time field is the single source of truth. Medication's `started_at` is optional: while absent, `occurred_at` stays null and the standard `created_at` fallback applies everywhere (effectively add-time — no timestamp is written); the invariant takes over the first time `started_at` is set. Payload time fields (`starts_at`, `ends_at`, `started_at`, `stopped_at`) remain detail, never promoted.
